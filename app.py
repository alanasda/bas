    ...
    try:
        ...
        if result.data:
            ...
            if usuario["pagamento_confirmado"]:
                ...
                return resposta_json({"success": False, "message": "E-mail já registrado."}, 400)
        else:
            senha_hash = ph.hash(senha)
...
def liberar_acesso():
    try:
        ...
        corpo_email = f"""
        <html><body>
        <h2>🔥 Olá, Visionário!</h2>
        <p>Seus módulos foram liberados:</p>
        <ul>
            {''.join(f'<li>{m}</li>' for m in modulos_ids)}
        </ul>
        <p><a href="https://cyberflux.onrender.com">Acessar plataforma</a></p>
        </body></html>
        """
        yag.send(to=email, subject="🎉 Acesso Liberado", contents=corpo_email)
...
def ping():
    return resposta_json({"success": True, "message": "Servidor está ativo"})

if __name__ == "__main__":
    try:
        socket.gethostbyname(EMAIL_CONFIG["smtp_server"])
        logger.info("SMTP disponível.")
